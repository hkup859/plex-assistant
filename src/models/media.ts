...
export const findAllUnprocessedMedia = async (): Promise<any> => {
    try {
        ...
        return await Media.find({ detailsLink: { $exists: true }, error: false, title: { $exists: false }, year: { $exists: false }, length: { $exists: false }}) // TODO, add other fields & perhaps reprocess errors a certain number of times
    } catch (err) {
        console.log(`Fatal Error In Media Model findMedia: ${err}`)
        throw err
    }
